...
gcc -o loadGenerator_10 loadGenerator_10.c -lpthread

# Start clients in the background
for ((i = 0; i < num_clients; i++)); do
    gnome-terminal -- bash -c "./loadGenerator_10 "$serverIP" "$port" "$program" "$num_iterations" "$sleep_time" "$i" "$num_clients" "$tos"; exit" &
