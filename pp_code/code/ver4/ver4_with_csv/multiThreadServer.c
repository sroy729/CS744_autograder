...
void start_function(struct Task *m) 
{
    ...
    char con_type[MAX_FILE_SIZE];
    int ct = recv(clientSocket, con_type, sizeof(con_type), 0);
    if (ct <= 0) {
        close(clientSocket);
        // break;
    }
    // int conntype;
    // memcpy(&conntype, con_type, sizeof(con_type));
    printf("conntype: %s\n", con_type);
    char new_conn[50] = "NEW";
    char status_conn[50] = "STATUS";
    int conntype;

    if (sizeof(con_type) != sizeof(new_conn) || memcmp(new_conn, con_type, sizeof(con_type)) != 0) {
        // printf("Difference found\n");
        conntype = 1; // status check connection
    }
    else
    {
        conntype = 0; // new connection
    }
    ...
    if(conntype == 0)
    {
        ...
        fclose(sourceFile);
        // insert row into db and return the request id to the client.
        char dbms[100];
        strcpy(dbms, DBMS_DIR);
        strcat(dbms, "dbms.csv");

        FILE* fp = fopen(dbms, "a+r");
        if (!fp)
		    printf("Can't open file\n");

        int gradingstatus = 0; //0-not done, 1- completed
        int clientcheckstatus = 0; //0-client checked the status, 1- not checked
        char results = 'N'; // result of grading is N, if not graded yet, P for pass, O for output error, R for runtime error, C for compile error

        fprintf(fp, "%ld, %d, %d, %c\n", request_id, gradingstatus, clientcheckstatus, results);
        fclose(fp);

        char msg[MAX_BUFFER_SIZE];
        // char msg[MAX_BUFFER_SIZE] = itoa(request_id);
        sprintf(msg, "%ld", request_id);
        ...
        // then call compile and run for grading and store results in the db        
        result = compileAndRun(src, num_clients);
        printf("result: %d\n", result);
    }
    else if(conntype == 1)
    {
        char requestid[MAX_FILE_SIZE];
        int rs = recv(clientSocket, requestid, sizeof(requestid), 0);
        if (rs <= 0) {
            close(clientSocket);
            // break;
        }
        int req;
        memcpy(&req, requestid, sizeof(requestid));
        printf("reqid: %d\n", req);
        ...
        char dbmsbuffer[1024];
		int row = 0;
		int column = 0;
        int gradingstat;
        char gradingresult='N';

		while (fgets(dbmsbuffer,1024, fp)) 
        {
            // printf("%ld\n", sizeof(buffer));
			column = 0;
			row++;
        ...
        if(atoi(gradingstat) == 1)
        {
            ...
            else if (gradingresult == 'N') 
            {
                char msg[MAX_BUFFER_SIZE] = "UNGRADED";
                // fd1 = open("runtimeError.txt", O_RDONLY, 0644);
                printf("Ungraded Msg Sent: %s", msg);
                send(clientSocket, msg, sizeof(msg), 0);
                printf("\nOUTPUT ERROR\n");
                bzero(msg, MAX_BUFFER_SIZE);
            }
        ...
        {
            char gradmsg[MAX_BUFFER_SIZE];
            //  = atoi(gradingstat);
            sprintf(gradmsg, "%d", gradingstat);
            send(clientSocket, gradmsg, sizeof(gradmsg), 0); 
            bzero(gradmsg, MAX_BUFFER_SIZE);
        }
    }
